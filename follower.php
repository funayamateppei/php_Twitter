...
$sql = 'SELECT * FROM follow_table LEFT OUTER JOIN (SELECT id, username FROM user_table) AS user_table2 ON follow_table.my_id = user_table2.id WHERE follow_table.your_id = :your_id';
...
foreach ($row as $v) {
  ...
  $stmt = $pdo->prepare($sql);
  $stmt->bindValue(':user_id', $v['my_id'], PDO::PARAM_INT);
  $stmt->execute();
  ...
  $htmlElements .= "
    <div class='item'>
      ...
      <div class='sentence'>
          ...
          <div>
            <a href='./follower_update.php?id={$v['id']}'>{$follow}</a>
